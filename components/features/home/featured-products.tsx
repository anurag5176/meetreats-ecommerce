...
export function FeaturedProducts() {
  ...
  const featuredProducts = [
    {
      ...
      description:
        "Activated almonds coated in rich dark cocoa and unrefined jaggery — indulgence with intention.",
    ...
    },
    {
      ...
      description:
        "Dehydrated plum pieces with a hint of cinnamon, naturally sweet and tangy",
      image: "/dried-jamun-plums-purple.jpg",
      slug: "plum-dusk-bites",
    },
    {
      id: 3,
      name: "Matcha Almond Medley",
      description: "Earthy matcha meets activated almonds for mindful energy.",
  ...
  return (
    <section
      ref={sectionRef}
      className="pt-7 pb-20 sm:pb-24 lg:pb-32 bg-soft-cream"
    >
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-16 lg:gap-20 xl:gap-24 mb-10 sm:mb-12 lg:mb-16 relative">
          ...
          <div
            className={`hidden lg:block absolute left-1/3 top-0 bottom-0 w-px bg-gradient-to-b from-transparent via-royal-gold/40 to-transparent transform -translate-x-1/2 transition-all duration-700 ease-out ${
              isSectionVisible
                ? "opacity-100 translate-y-0"
          ...
          ></div>
          <div
            className={`hidden lg:block absolute left-2/3 top-0 bottom-0 w-px bg-gradient-to-b from-transparent via-royal-gold/40 to-transparent transform -translate-x-1/2 transition-all duration-700 ease-out ${
              isSectionVisible
                ? "opacity-100 translate-y-0"
                : "opacity-0 translate-y-4"
            }`}
          ...
          {featuredProducts.map((product, index) => (
            <div
              key={product.id}
              className={`group text-center flex flex-col h-full transition-all duration-700 ease-out ${
                isSectionVisible
                  ? "opacity-100 translate-y-0"
                  : "opacity-0 translate-y-8"
              }`}
