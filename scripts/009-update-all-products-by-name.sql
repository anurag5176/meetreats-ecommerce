-- Alternative: Update by product name (case-insensitive)
-- This will update products that match the name, regardless of slug
...
SET 
    ...
    featured = true
WHERE LOWER(name) LIKE '%cocoa cloud crunch%' OR LOWER(name) = 'cocoa cloud crunch';

-- Plum Dusk Bites / Jamun
UPDATE products
SET 
    ...
    featured = true
WHERE LOWER(name) LIKE '%plum%' OR LOWER(name) LIKE '%jamun%' OR LOWER(name) LIKE '%dusk%';

-- Tuscan Herbs / Herb Seasoned
UPDATE products
SET 
    ...
    is_provisional_nutrition = false,
    featured = false
WHERE LOWER(name) LIKE '%herb%' OR LOWER(name) LIKE '%tuscan%';

-- Amber Aura / Chai Spiced / Golden
UPDATE products
SET 
    ...
    is_provisional_nutrition = false,
    featured = false
WHERE LOWER(name) LIKE '%amber%' OR LOWER(name) LIKE '%aura%' OR LOWER(name) LIKE '%chai%' OR LOWER(name) LIKE '%golden%' OR LOWER(name) LIKE '%turmeric%';
...
SET 
    ...
    is_provisional_nutrition = false,
    featured = true
WHERE LOWER(name) LIKE '%matcha%' OR LOWER(name) LIKE '%muse%';

-- Strawberry / Ruby Blush
UPDATE products
SET 
    ...
    is_provisional_nutrition = true,
    featured = true
WHERE LOWER(name) LIKE '%strawberry%' OR LOWER(name) LIKE '%ruby%' OR LOWER(name) LIKE '%blush%';
